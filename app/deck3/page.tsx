...
function Footer({ number, inverse = false }: { number: number; inverse?: boolean }) {
  return (
    <footer className={`deck-footer ${inverse ? "inverse" : ""}`}>
      <span>{summitContact.web}</span>
      <span>{String(number).padStart(2, "0")}</span>
    </footer>
  );
}

function Page({ number, eyebrow, title, intro, children, tone = "light", backgroundImage, className = "" }: PageProps) {
  ...
  return (
    ...
    >
      ...
      <Footer number={number} inverse={inverse} />
    </section>
  );
}
...
export default function Deck3Page() {
  return (
    <main className="deck3-root" id="deck-root">
      <PDFDownloadButton />
      ...
      <section className="soq-page cover-page">
        ...
        <div className="cover-copy">
          ...
          <h1>Statement of<br />Qualifications</h1>
      ...
      <Page number={2} eyebrow="Document Guide" title="Contents" intro="A concise view of the people, capabilities and field resources behind every Summit project.">
        <div className="toc-grid">
          {[
            ["03", "One Company, Expanded Reach"], ["04", "Who We Are"], ["05", "Where We Work"],
            ["06", "Integrated Capabilities"], ["07", "Environmental & Geotechnical Drilling"], ["08", "Sonic Drilling"],
            ["09", "Direct Push, Auger, Rotary & Coring"], ["10", "Remediation & Injection"], ["11", "Construction & Vacuum Excavation"],
            ["12", "Geophysics & Utility Locating"], ["13", "PFAS Experience"], ["14", "Fleet & Operational Capacity"],
            ["15", "Health, Safety & Quality"], ["16", "Licenses & Industries"], ["17", "Representative Experience"], ["18", "Let's Get to Work"],
          ].map(([num, label]) => <div className="toc-row" key={num}><span>{num}</span><strong>{label}</strong></div>)}
        </div>
        <div className="toc-photo"><img src="/images/remediation/hero-banner.webp" alt="Summit field team" /></div>
      </Page>

      <Page number={3} eyebrow="Summit + PDS (A Summit Drilling company)" title="One Company. Expanded Reach." tone="photo" backgroundImage="/images/news/summit-pds-acquisition.webp">
      ...
      <Page number={4} eyebrow="Our Purpose" title="Built to Support Environmental Professionals" intro="We help consultants and engineers obtain reliable subsurface information and execute effective cleanup strategies.">
      ...
      <Page number={5} eyebrow="Multi-Regional Service" title="Local Knowledge. Connected Resources." intro="A growing network of field teams supports programs from the Northeast through the Southeast and Gulf Coast.">
      ...
      <Page number={6} eyebrow="Project Lifecycle" title="Integrated Capabilities" intro="A connected suite of field services reduces handoffs, simplifies coordination and supports projects from investigation through site closure.">
        <div className="process-line">
          {[
            ["01", "See", "Geophysics and utility locating"], ["02", "Investigate", "Environmental and geotechnical drilling"],
            ["03", "Characterize", "Sampling, logging and subsurface data"], ["04", "Treat", "Injection, remediation and construction"],
      ...
      <Page number={7} eyebrow="Core Capability" title="Environmental & Geotechnical Drilling" intro="Field-proven methods selected around lithology, access, sampling objectives and final data needs.">
      ...
      <Page number={8} eyebrow="Specialized Drilling" title="Sonic Drilling" tone="blue">
      ...
      <Page number={9} eyebrow="Right Tool. Right Formation." title="Direct Push, Auger, Rotary & Coring" intro="Multiple platforms allow Summit teams to adapt to site constraints without losing sight of the investigation objective.">
        <div className="method-matrix">
          {[
            ["Direct Push", "Rapid soil and groundwater sampling, microwells and direct injection."],
            ["Hollow Stem Auger", "Stable boreholes, split-spoon sampling and conventional well installation."],
            ["Air / Mud Rotary", "Deeper drilling, competent formations and larger-diameter installations."],
            ["Rock Coring", "High-quality core recovery for geotechnical and environmental characterization."],
          ].map(([title, text], i) => <article key={title}><span>0{i + 1}</span><h3>{title}</h3><p>{text}</p></article>)}
        </div>
        <div className="image-band three">
          <img src="/images/drilling/direct-push.webp" alt="Direct push rig" />
          <img src="/images/drilling/auger.webp" alt="Auger drilling" />
          <img src="/images/drilling/air-rotary.webp" alt="Rotary drilling" />
        </div>
      </Page>

      <Page number={10} eyebrow="From Design to Field Execution" title="Remediation & Injection Services" intro="Experienced crews translate remedial designs into safe, repeatable field programs with disciplined documentation.">
        <div className="photo-grid two">
      ...
      <Page number={11} eyebrow="Field Implementation" title="Environmental Construction & Vacuum Excavation" intro="Practical construction capability keeps environmental designs moving from paper to completion.">
        <div className="split-feature reverse">
          ...
          <div>
            ...
            <h3>Soft-dig / vacuum excavation</h3>
      ...
      <Page number={12} eyebrow="See Before You Drill" title="Geophysics & Utility Locating" intro="Multiple technologies are combined to improve subsurface understanding, reduce utility risk and guide focused investigation.">
      ...
      <Page number={13} eyebrow="Emerging Contaminants" title="PFAS Experience" tone="photo" backgroundImage="/images/projects/charlotte-airport-banner.webp">
      ...
      <Page number={14} eyebrow="Ready for the Field" title="Fleet & Operational Capacity" intro="A diverse equipment platform supports the formation, access constraints and production requirements of each assignment—without turning capability into a vanity counter.">
        <div className="equipment-grid">
          {[
            ["Sonic", "/images/drilling/sonic.webp"], ["Direct Push", "/images/drilling/direct-push.webp"],
            ["Auger & Rotary", "/images/drilling/auger.webp"], ["Injection", "/images/remediation/injection.webp"],
            ["Geophysics", "/images/geophysics/utility-locating.webp"], ["Construction", "/images/remediation/earthwork.webp"],
      ...
      <Page number={15} eyebrow="Our First Priority" title="Health, Safety & Quality" intro="Safety is a working system—visible in planning, training, equipment care, field leadership and daily decisions.">
        <div className="safety-hero"><img src="/images/health-safety-banner.webp" alt="Summit health and safety team" /><Quote>Every employee has a role in protecting coworkers, clients, communities and the environment.</Quote></div>
      ...
      <Page number={16} eyebrow="Qualified for Complex Work" title="Licenses & Industries Served" intro="Regional credentials and sector experience support work in regulated, sensitive and operationally demanding environments.">
        <div className="license-layout">
          ...
          <div>
            <h3>Industries</h3>
            <div className="industry-tags">{["Environmental Consulting", "Engineering", "Industrial", "Energy", "Transportation", "Airports", "Government", "Military", "Petroleum", "Utilities", "Mining", "Landfills", "Commercial Development", "Water Resources"].map(x => <span key={x}>{x}</span>)}</div>
          </div>
        </div>
      ...
      <Page number={17} eyebrow="Representative Experience" title="Complex Sites. Practical Execution." intro="A few examples of the environments and project conditions Summit teams are equipped to manage.">
      ...
      <section className="soq-page closing-page">
        ...
        <div className="closing-copy">
          ...
          <div className="contact-card">
            <div><span>Email</span><a href={`mailto:${summitContact.email}`}>{summitContact.email}</a></div>
            <div><span>Phone</span><a href={`tel:${summitContact.phone.replace(/-/g, "")}`}>{summitContact.phone}</a></div>
            <div><span>Web</span><a href="https://summitdrilling.com">{summitContact.web}</a></div>
          </div>
          <p className="closing-pds">PDS <strong>(A Summit Drilling company)</strong></p>
        </div>
        <Footer number={18} inverse />
      </section>
    </main>
  );
}
